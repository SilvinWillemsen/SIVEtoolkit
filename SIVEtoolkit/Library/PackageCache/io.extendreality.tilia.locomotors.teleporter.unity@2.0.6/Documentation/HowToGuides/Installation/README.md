...
The `Locomotors.Teleporter.Instant` and `Locomotors.Teleporter.Dash` prefabs provide a way of moving a target GameObject to a specific point in the scene, which is an effective form of locomotion for spatial environtments. This prefab can be included in a [Unity] software project via the [Unity Package Manager].
...
The `Tilia Locomotors Teleporter Unity` package will now be available in your Unity project `Packages` directory ready for use in your project.
