...
The Pointer Interactors allow the ability to interact with interactable objects via ObjectPointers and come in the form of the following prefabs:

* `Interactions.PointerInteractors.DistanceGrabber` - Emits a hidden straight pointer from the interactor and identifies any distant interactable that can be transitioned to the interactor upon grab.
...
* Adjust the [project manifest file][Project-Manifest] `manifest.json` in a text editor.
  ...
  * Add `io.extendreality.tilia.interactions.pointerinteractors.unity` to `dependencies`, stating the latest version.
  ...
  ```json
  {
    ...
    "dependencies": {
      "io.extendreality.tilia.interactions.pointerinteractors.unity": "X.Y.Z",
      ...
    }
  }
  ```
...
The `Tilia Interactions PointerInteractors Unity` package will now be available in your Unity project `Packages` directory ready for use in your project.
