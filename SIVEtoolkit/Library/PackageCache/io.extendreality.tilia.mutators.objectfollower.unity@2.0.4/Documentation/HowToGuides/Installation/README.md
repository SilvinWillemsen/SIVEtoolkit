...
The `Mutators.ObjectFollower` prefab provides the ability to have one target GameObject follow a source GameObject with 1:1 mapping around a [Unity] scene. This prefab can be included in a Unity software project via the [Unity Package Manager].
...
* Adjust the [project manifest file][Project-Manifest] `manifest.json` in a text editor.
  ...
  * Add `io.extendreality.tilia.mutators.objectfollower.unity` to `dependencies`, stating the latest version.
  ...
  ```json
  {
    ...
    "dependencies": {
      "io.extendreality.tilia.mutators.objectfollower.unity": "X.Y.Z",
      ...
    }
  }
  ```
...
The `Tilia Mutators ObjectFollower Unity` package will now be available in your Unity project `Packages` directory ready for use in your project.
